...
## 2.0.12 - fix: v2 cannot activate inside v1 processes + console.log verbose gate (2026-09-18)
...
- **console.log を全箇所で verbose ゲート付きに変更**: `DAILY_LOGBOOK_DEBUG=1`（または `VERBOSE=1` / `LOG_EVENTS=1`）がセットされているときだけ stdout に出力。ファイルログ（`daily-logbook-v2.log`）は常時有効
...
- 全 `console.log` を `DAILY_LOGBOOK_DEBUG=1` ゲート付きに変更（6箇所: v1 plugin loaded, v1 event verbose, v2 startup, v2 subscribe loop×2, v2 log sink info）
